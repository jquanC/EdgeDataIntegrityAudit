...
public class PDPeEDIAudit extends AuditComponent {
    ...
    public PDPeEDIAudit(String filePath, String replicaPath, int BLOCK_SIZE, int CHA_LEN) {
    ...
    public boolean Verify(ProofData[] proofData, BigInteger[][] allSelectBigNumBlocks, ChallengeData[] challengeSet, Key key) throws IOException {
        ...
        boolean intactPOI = true;

        /**
         * in PDPe,check each POI to simulate the actual PDP scheme,so we don't need to calculate combineTag
        ...
        for (int t = 0; t < esNum; t++) {
            ...
            checkBlock = g.modPow(tag,N);
            intactPOI = checkBlock.equals(proofData[t].combineTag);
            if (intactPOI == false) {
                System.out.println("Data in "+t+"_th ES is corrupted.");
                break;
            }
        }
